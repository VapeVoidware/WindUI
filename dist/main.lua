...
do
		...
		function b.New(g, h, i, j, k, n, o)
			...
			local r = e("TextButton", {
				...
				d.NewRoundFrame(p, not o and "SquircleOutline" or "SquircleOutline2", {
					...
					ImageTransparency = j == "Primary" and 0.95 or 0.85,
			...
			d.AddSignal(r.MouseEnter, function()
				f(r.Frame, 0.047, { ImageTransparency = 0.95 }):Play()
			end)
			d.AddSignal(r.MouseLeave, function()
				f(r.Frame, 0.047, { ImageTransparency = 1 }):Play()
			end)
		...
		function f.Create(g)
			...
			if not g then
				h.UIElements.FullScreen = d("Frame", {
					...
					Parent = f.Parent
						or (
							f.Window
							and f.Window.UIElements
							and f.Window.UIElements.Main
							and f.Window.UIElements.Main.Main
						),
				}, {
					d("UICorner", {
			...
			function h.Open(i)
				...
				task.spawn(function()
					...
					if not g then
						e(h.UIElements.FullScreen, 0.1, { BackgroundTransparency = 0.3 }):Play()
					end
	...
	function a.l()
		...
		function b.New(h)
			...
			d.AddSignal(q.TextButton.MouseEnter, function()
				f(q.TextButton, 0.1, { BackgroundTransparency = 0.93 }):Play()
			end)
			d.AddSignal(q.TextButton.MouseLeave, function()
				f(q.TextButton, 0.1, { BackgroundTransparency = 1 }):Play()
			end)
	...
	function a.m()
		...
		function b.New(g, h)
			...
			function i.Open(o)
				...
				f(n, 0.16, { GroupTransparency = 0 }, Enum.EasingStyle.Quint, Enum.EasingDirection.Out):Play()
				f(k, 0.18, { Scale = 1 }, Enum.EasingStyle.Quint, Enum.EasingDirection.Out):Play()
			end

			function i.Close(o)
				f(n, 0.2, { GroupTransparency = 1 }, Enum.EasingStyle.Quint, Enum.EasingDirection.Out):Play()
				f(k, 0.2, { Scale = 0.9 }, Enum.EasingStyle.Quint, Enum.EasingDirection.Out):Play()
	...
	function a.s()
		...
		function g.New(i, j)
            ...
            local suc, err = pcall(function()
                k.SliderFrame.UIElements.Container:FindFirstChildOfClass("UIListLayout"):Destroy()
                k.UIElements.SliderContainer.Name = "other"
                k.UIElements.SliderContainer.Size = UDim2.fromScale(0.5, 0)
                local sinx
                for _, v in pairs(k.SliderFrame.UIElements.Container:GetChildren()) do
                    if v:FindFirstChild("TextBox") then
                        sinx = v
                    else
                        v.Size = UDim2.fromScale(0.5, 0)
                    end
                end
                k.UIElements.SliderContainer.Name = "Frame"
                sinx.Size = UDim2.fromScale(0.5, 0)
                sinx.Position = UDim2.fromScale(1, 0.5)
                sinx.AnchorPoint = Vector2.new(1, 0.5)
                sinx.TextBox.Size = UDim2.fromOffset(30, 0)
                sinx.Frame.Size = UDim2.new(1, -38, 0, 4)
            end)
	...
	function a.t()
		...
		function h.New(j, k)
			...
			n.UIElements.Keybind.Size =
				UDim2.new(0, 24 + n.UIElements.Keybind.Frame.Frame.TextLabel.TextBounds.X, 0, 42)
					...
					if n.CanChange then
						...
						p = b.InputBegan:Connect(function(q)
							...
							s = b.InputEnded:Connect(function(t)
								if
									t.KeyCode.Name == r
									or r == "MouseLeft" and t.UserInputType == Enum.UserInputType.MouseButton1
									or r == "MouseRight" and t.UserInputType == Enum.UserInputType.MouseButton2
								then
									n.Picking = false
...
.New
		...
		function g.New(j, k)
            ...
            pcall(function()
                p.Parent:FindFirstChildOfClass("UIListLayout"):Destroy()
                p.Position = UDim2.new(1, 0, 0, 0)
                p.AnchorPoint = Vector2.new(1, 0)
                p.Size = UDim2.new(0.5, 0, 0, n.Type == "Input" and 42 or 148)
                p.Name = "e"
                p.Parent.Frame.Position = UDim2.fromScale(0, 0.5)
                p.Parent.Frame.Size = UDim2.fromScale(0.5, 0)
                p.Parent.Frame.AnchorPoint = Vector2.new(0, 0.5)
                p.Name = "Frame"
            end)

            pcall(function()
                local sick = p.Parent.Parent:FindFirstChildOfClass("UIPadding")
                sick.PaddingBottom = UDim.new(0, 5)
                sick.PaddingTop = UDim.new(0, 5)
            end)
	...
	function a.v()
		...
		function n.New(o, p)
			...
			q.UIElements.Dropdown.Frame.Frame.TextLabel.Size =
				UDim2.new(1, q.UIElements.Dropdown.Frame.Frame.TextLabel.Size.X.Offset - 18 - 12 - 12, 0, 0)

			q.UIElements.Dropdown.Size = UDim2.new(1, 0, 0, 40)

            q.UIElements.Dropdown.Size = UDim2.new(0.5, 0, 0, 40)
            q.UIElements.Dropdown.AnchorPoint = Vector2.new(1, 0)
            pcall(function()
                Instance.new("UIScale", q.UIElements.Dropdown).Scale = 0.9
                q.UIElements.Dropdown.Position = UDim2.fromScale(1, 0.1)
            end)
            pcall(function()
                q.UIElements.Dropdown.Parent:FindFirstChildOfClass("UIListLayout"):Destroy()
            end)
            pcall(function()
                local quack = q.UIElements.Dropdown.Parent.Frame
                quack.AnchorPoint = Vector2.new(0, 0.5)
                quack.Position = UDim2.fromScale(0, 0.5)
                quack.Size = UDim2.fromScale(0.5, 0)
                --Instance.new("UIAspectRatioConstraint", q.UIElements.Dropdown.Parent.Parent).AspectRatio = 4.8
            end)
            pcall(function()
                local sick = q.UIElements.Dropdown.Parent.Parent:FindFirstChildOfClass("UIPadding")
                sick.PaddingBottom = UDim.new(0, 5)
                sick.PaddingTop = UDim.new(0, 5)
            end)
			...
			local function RecalculateListSize()
				if #q.Values > 10 then
					...
					q.UIElements.MenuCanvas.Size = UDim2.fromOffset(
						q.UIElements.MenuCanvas.AbsoluteSize.X,
						q.UIElements.UIListLayout.AbsoluteContentSize.Y + (n.MenuPadding * 2)
					)
				end
			end
			...
			function q.Refresh(s, t)
				...
				for w, x in next, t do
					...
					if y.Selected then
						y.UIElements.TabItem.ImageTransparency = 0.95
						y.UIElements.TabItem.Highlight.ImageTransparency = 0.75
					...
					h.AddSignal(y.UIElements.TabItem.MouseButton1Click, function()
						if q.Multi then
							if not y.Selected then
								y.Selected = true
								j(y.UIElements.TabItem, 0.1, { ImageTransparency = 0.95 }):Play()
								j(y.UIElements.TabItem.Highlight, 0.1, { ImageTransparency = 0.75 }):Play()
								...
								j(y.UIElements.TabItem.Frame.TextLabel, 0.1, { TextTransparency = 0.4 }):Play()
							...
							for z, A in next, q.Tabs do
								...
								j(A.UIElements.TabItem.Frame.TextLabel, 0.1, { TextTransparency = 0.5 }):Play()
								A.Selected = false
							end
							y.Selected = true
							j(y.UIElements.TabItem, 0.1, { ImageTransparency = 0.95 }):Play()
			...
			h.AddSignal(b.InputBegan, function(s)
				...
				then
					...
					if
						...
						and q.Opened
						and (e.X < t.X or e.X > t.X + v.X or e.Y < (t.Y - 20 - 1) or e.Y > t.Y + v.Y)
					then
						q:Close()
					end
				end
	...
	function a.w()
		...
		function b.run(k)
			...
			for t, v in ipairs(n) do
				...
				if w then
					local x =
						string.format('<font color = "#%s">%s</font>', w:ToHex(), v:gsub("<", "&lt;"):gsub(">", "&gt;"))
	...
	function a.x()
		...
		function b.New(j, k, n, o, p)
			...
			e.AddSignal(t.MouseEnter, function()
				h(t.Button, 0.05, { ImageTransparency = 0.95 }):Play()
				h(t.Button.UIScale, 0.05, { Scale = 0.9 }):Play()
			end)
			e.AddSignal(t.InputEnded, function()
				h(t.Button, 0.08, { ImageTransparency = 1 }):Play()
				h(t.Button.UIScale, 0.08, { Scale = 1 }):Play()
			end)
	...
	function a.B()
		...
		function i.New(j)
			...
			if k.Icon then
				...
				s.ImageLabel.ImageTransparency = not k.Locked and 0 or 0.7
				q = -30
			end
			...
			ab.AddSignal(k.UIElements.Main.MouseEnter, function()
				if not k.Locked then
					b(k.UIElements.Main.Frame, 0.08, { ImageTransparency = 0.97 }):Play()
				end
			end)
	...
	function a.C()
		...
		function aa.New(g, h, i, j)
			...
			local n
			if k.Icon then
				n = ab.Image(k.Icon, k.Icon, 0, i, "Section", true, k.IconThemed)

				n.Size = UDim2.new(0, k.IconSize, 0, k.IconSize)
				n.ImageLabel.ImageTransparency = 0.25
			end
			...
			local p = ac("Frame", {
				...
				ac("TextButton", {
					...
					ac("TextLabel", {
						Text = k.Title,
						TextXAlignment = "Left",
						Size = UDim2.new(1, n and (-k.IconSize - 10) * 2 or (-k.IconSize - 10), 1, 0),
						ThemeTag = {
							TextColor3 = "Text",
						},
						FontFace = Font.new(ab.Font, Enum.FontWeight.SemiBold),
		...
		function aa.new(e, g, h)
			...
			local function CreateSearchTab(q, r, s, t, v, w)
				...
				x.Main.Size = UDim2.new(
					...
					0,
					0,
					x.Main.Frame.Desc.Visible
							and (((i.Padding - 2) * 2) + x.Main.Frame.Title.TextBounds.Y + 6 + x.Main.Frame.Desc.TextBounds.Y)
						or (((i.Padding - 2) * 2) + x.Main.Frame.Title.TextBounds.Y)
				)

				ab.AddSignal(x.Main.MouseEnter, function()
					b(x.Main, 0.04, { ImageTransparency = 0.95 }):Play()
				end)
				ab.AddSignal(x.Main.InputEnded, function()
					b(x.Main, 0.08, { ImageTransparency = 1 }):Play()
				end)
						...
						if x.Elements and next(x.Elements) ~= nil then
							for A, B in next, x.Elements do
								...
								CreateSearchTab(
									...
									B.Desc,
									C,
									z:FindFirstChild("ParentContainer") and z.ParentContainer.Frame or nil,
									false,
									function()
										i:Close()
										e:SelectTab(w)
	...
	function a.F()
		...
		return function(n)
			...
			if o.User.Enabled then
				...
				if o.User.Callback then
					...
					ac.AddSignal(w.MouseEnter, function()
						e(w.UserIcon, 0.04, { ImageTransparency = 0.95 }):Play()
						e(w.Outline, 0.04, { ImageTransparency = 0.85 }):Play()
					end)
			...
			function o.CreateTopbarButton(B, C, D, E, F, G)
				local H = ac.Image(D, D, 0, o.Folder, "TopbarIcon", true, G)
				...
				ac.AddSignal(I.MouseEnter, function()
					e(I, 0.15, { ImageTransparency = 0.93 }):Play()
					e(I.Outline, 0.15, { ImageTransparency = 0.75 }):Play()
				end)
				ac.AddSignal(I.MouseLeave, function()
					e(I, 0.1, { ImageTransparency = 1 }):Play()
					e(I.Outline, 0.1, { ImageTransparency = 1 }):Play()
				end)

				return I
			end

			local B = ac.Drag(o.UIElements.Main, { o.UIElements.Main.Main.Topbar, z.Frame }, function(B, C)
				if not o.Closed then
					if B and C == z.Frame then
						e(z, 0.1, { ImageTransparency = 0.35 }):Play()
					else
						e(z, 0.2, { ImageTransparency = 0.8 }):Play()
					end
				end
			end)
			...
			task.spawn(function()
				if o.Icon then
					local D = ac.Image(o.Icon, o.Title, 0, o.Folder, "Window", true, o.IconThemed)
			...
			function o.ToggleFullscreen(F)
				...
				e(
					o.UIElements.Main,
					0.45,
					{ Size = G and E or UDim2.new(1, -20, 1, -72) },
					Enum.EasingStyle.Quint,
					Enum.EasingDirection.Out
				):Play()
				...
				e(
					o.UIElements.Main,
					0.45,
					{ Position = G and D or UDim2.new(0.5, 0, 0.5, 26) },
					Enum.EasingStyle.Quint,
					Enum.EasingDirection.Out
				):Play()
			...
			o:CreateTopbarButton("Minimize", "minus", function()
				...
				local F = v and "Press " .. o.ToggleKey.Name .. " to open the Window"
					or "Click the Button to open the Window"
			...
			function o.Open(F)
				task.spawn(function()
					...
					e(t, 0.25, { ImageTransparency = 0.7 }, Enum.EasingStyle.Quint, Enum.EasingDirection.Out):Play()
					if UIStroke then
			...
			function o.SetUIScale(F, G)
				n.WindUI.UIScale = G
				e(n.WindUI.ScreenGui.UIScale, 0.2, { Scale = G }, Enum.EasingStyle.Quint, Enum.EasingDirection.Out):Play()
			end

			do
				if
					(ab.ViewportSize.X - 40 < o.UIElements.Main.AbsoluteSize.X)
					or (ab.ViewportSize.Y - 40 < o.UIElements.Main.AbsoluteSize.Y)
				then
			...
			function o.Dialog(J, K)
				...
				local O
				if K.Icon then
					O = ac.Image(K.Icon, L.Title .. ":" .. K.Icon, 0, o, "Dialog", true, K.IconThemed)
					O.Size = UDim2.new(0, 22, 0, 22)
					O.Parent = N
				end
